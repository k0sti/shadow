...
3. `ui/crates/shadow-compositor` is the fast Linux desktop host for shell iteration and the main Smithay bring-up path.
4. `scripts/ui_run.sh` is the operator wrapper that stages runtime bundles, launches the desktop compositor, and can auto-open a selected app.
...
15. `just pixel-linux-audio-spike` is the first Linux-direct audio probe on the rooted phone: it stages a small ALSA tone player plus copied ALSA userspace into the GNU envelope, runs that probe under the same rooted display-takeover seam, and reports whether direct Linux playback can reach the device speaker without any Android bridge.
16. `just runtime-app-keyboard-smoke` is the narrow bundled-host rung: it proves focus, keydown, text input, selection metadata, and blur semantics through the live runtime session.
17. `just runtime-app-nostr-gm-smoke` is the async host rung: it proves a click can drive the current Nostr publish seam and surface a completion state back into the app.
18. `just runtime-app-nostr-timeline-smoke` is the main host proving ground: it proves relay sync, keyboard-driven compose, and cold-restart cache reload through the same bundled runtime contract.
19. `just pixel-shell-drm` is now the primary rooted Pixel shell rung: stage the counter, timeline, and podcast runtime bundles plus the GNU-wrapped helper, boot the guest compositor into the home scene, and present the real shell/home path on the phone panel. `just run app=podcast target=pixel` now uses that same shell rung and asks the shell to open `podcast`.
20. `just pixel-shellctl ...` is the rooted Pixel shell control seam: it talks to the guest compositor's `shadow-control.sock` over `adb shell` plus `nc -U`, so operators can query state, go home, or launch an app without falling back to touch-only QA.
21. `just pixel-shell-timeline-smoke` is the first rooted lifecycle proof on that shell lane: start `timeline`, go home, and reopen it through the live shell control path.
...
25. `just runtime-app-sound-smoke` plus `just pixel-runtime-app-sound-drm` are the first runtime-audio seam: a runtime app now calls `Shadow.os.audio`, the host proves the handle API against an in-memory backend and locally guards the `linux_spike` stdio contract, and the rooted Pixel sound lane swaps to the Linux-direct helper without adding a JVM bridge.
26. That sound lane is no longer tone-only. The sound demo now uses a checked-in MP3 fixture under `runtime/app-sound-smoke/assets/`, runtime bundle prep copies sibling `assets/` beside `bundle.js` on host and Pixel, and rooted Pixel auto-click runs prove the runtime host can spawn the file-backed Linux helper and reach audible playback on the real panel path.
...
22. Runtime audio now has its first vertical seam too. `runtime/app-runtime/shadow_runtime_os.js` exports `Shadow.os.audio`, `rust/runtime-audio-host` injects that API into the embedded runtime host, host smokes use a small in-memory backend, and the rooted Pixel sound app flips to a sound-only no-`chroot` launcher so a spawned Linux helper can still reach `/dev/snd`.
23. The first staged file-backed proof is now landed on top of that seam. `scripts/prepare_sound_demo_assets.sh` validates the checked-in MP3 fixture under `runtime/app-sound-smoke/assets/`, the runtime sound app receives that as a bundle-relative `file` source through `SHADOW_RUNTIME_APP_CONFIG_JSON`, `runtime-audio-host` resolves it via `SHADOW_RUNTIME_BUNDLE_DIR`, and the Linux helper decodes the file in-process before writing PCM to ALSA.
