    ...
    pub fn process_input_event<I: InputBackend>(&mut self, event: InputEvent<I>) {
        match event {
                ...
                let intent = keyboard
                    .input(
                        ...
                        |_, _, handle| {
                            ...
                            match event.state() {
                                KeyState::Pressed => {
                                    if keysym == keysyms::KEY_Home && has_foreground_app {
            ...
            InputEvent::PointerMotionAbsolute { event, .. } => {
                ...
                match under {
                    Some(_) => self.handle_shell_event(ShellEvent::PointerLeft),
                    None => self.handle_shell_event(ShellEvent::PointerMoved {
                        x: position.x as f32,
                        y: position.y as f32,
                    }),
                }
